#!/usr/bin/env python3
"""Kreditor page 581 blob upload state machine (2k chunks)."""
...
PENDING = Path("/workspace/.cursor/kreditor-2k-mcp-pending.json")
...
def load_state() -> dict:
    ...
    total = len(list(CHUNKS.glob("*.json")))
    return {"index": 0, "blob_id": None, "sha256": None, "bytes_total": None, "total": total}
...
def main() -> int:
    ...
    cmd = sys.argv[1]
    st = load_state()
    total = st.get("total") or len(list(CHUNKS.glob("*.json")))
