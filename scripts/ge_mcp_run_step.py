#!/usr/bin/env python3
"""Load google-earth blob append args for step N; write to /tmp for agent mcp_call_tool.

Usage:
  python3 scripts/ge_mcp_run_step.py STEP [BLOB_ID]
...
def main() -> int:
    if len(sys.argv) < 2:
        print("usage: ge_mcp_run_step.py STEP [BLOB_ID]", file=sys.stderr)
        return 2
    step = int(sys.argv[1])
    blob_id = sys.argv[2] if len(sys.argv) > 2 else ""
    args = load_step(step, blob_id)
    out = Path(f"/tmp/ge-mcp-run-step{step:02d}.json")
