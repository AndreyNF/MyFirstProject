#!/usr/bin/env python3
"""Load MCP append args for step N. Usage: vs-komp-mcp-load-args.py STEP [BLOB_ID]"""
...
def main() -> int:
    if len(sys.argv) < 2:
        print("usage: vs-komp-mcp-load-args.py STEP [BLOB_ID]", file=sys.stderr)
        return 2
    step = int(sys.argv[1])
    ...
    if step > 0:
        ...
        args.pop("reset", None)
    out = Path(f"/tmp/vs-komp-mcp-live-step{step}.json")
