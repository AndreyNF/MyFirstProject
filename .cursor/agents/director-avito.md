...
Пайплайн **лонгридов** (Кирилл → Коля → Женя → …) — агент `director` на ветке `origin/cursor/a3-a2ec`.  
Общий контекст сайта: `shared/legis24-site-context.md`.  
Тема лонгрида может породить SKU: после публикации статьи Директор Avito запускается с нишей из H1.
